...
insert into meme
	...
	select
		...
		meme_bg.fk				as meme_background_fk,
		meme_type.fk			as lv_meme_type_fk,
	...
	from
		...
		(
			select distinct
				...
				crawler_img_desc 	 as desc
			from
				crawler_background
		) as crawler_bg_filename
		on (upper(crawler_bg_desc.desc) = upper(crawler_bg_filename.desc)
